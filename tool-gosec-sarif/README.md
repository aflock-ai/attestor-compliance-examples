...
of `gosec` ([gosec](https://github.com/securego/gosec)) using rookery's `sarif` attestor.
...
cilock run --step gosec-scan \
  ...
  -- bash -c "cp $(echo 'gosec -fmt=sarif -out=gosec.sarif ./...' | grep -oE '[^ ]+\.sarif|[^ ]+\.json' | head -1) gosec-product.sarif"
```
